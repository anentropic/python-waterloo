...
docstring_type = st.sampled_from(
    [
        DocType("SomeClass", {"SomeClass": (TypeDef.BARE_NAME, TypeSrc.USER)}, {},),
        DocType(
            "my.module.SomeClass",
...
def SomeClass_import_f(draw, type_def: TypeDef) -> ImportT:
    imports = (
        ...
        NonAmbiguousImport(
            "from my.module import OtherClass, SomeClass, AnotherClass",
    ...
    if type_def is TypeDef.DOTTED_PATH:
        imports += (
            NonAmbiguousImport("import my.module", ImportType.PACKAGE_ONLY),
            AmbiguousImport("from my.module import *", Ambiguity.STAR_IMPORT),
            AmbiguousImport("from other.module import SomeClass", Ambiguity.NAME_CLASH),
            AmbiguousImport("from .. import SomeClass", Ambiguity.RELATIVE_IMPORT),
        )
...
def typing_import_f(draw, type_def: TypeDef) -> ImportT:
    ...
    if type_def is TypeDef.DOTTED_PATH:
        imports += (
            NonAmbiguousImport("import typing", ImportType.PACKAGE_ONLY),
            AmbiguousImport("from typing import *", Ambiguity.STAR_IMPORT),
            AmbiguousImport(
