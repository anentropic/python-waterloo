...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "Tuple[int, ...]",
            TypeAtom("Tuple", [TypeAtom("int", []), TypeAtom("...", [])]),
        ),
        (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Container[int]", TypeAtom("my.generic.Container", [TypeAtom("int", [])]), {}),
    ],
)
...
@pytest.mark.parametrize(
    "expected,example",
    [
        (set(), ArgTypes.factory(name=ArgsSection.ARGS, args=OrderedDict(),)),
        (
            set(),
            ArgTypes.factory(name=ArgsSection.ARGS, args=OrderedDict([("a", None)]),),
        ),
        (
            {"str"},
            ArgTypes.factory(
                ...
                args=OrderedDict(
                    [
                        (
                            "a",
                            TypeAtom(
                                "Dict", [TypeAtom("int", []), TypeAtom("str", [])]
                            ),
                        ),
                        ("b", TypeAtom("str", [])),
                        ("c", TypeAtom("datetime", [])),
...
@pytest.mark.parametrize(
    "expected,example",
    [
        (set(), ReturnType.factory(name=ReturnsSection.RETURNS, type_def=None,)),
        (
            {"str"},
            ReturnType.factory(
                name=ReturnsSection.RETURNS, type_def=TypeAtom("str", []),
            ),
        ),
        (
            {"Dict", "int", "str"},
            ReturnType.factory(
                ...
                type_def=TypeAtom("Dict", [TypeAtom("int", []), TypeAtom("str", [])]),
            ),
        ),
    ],
)
...
@pytest.mark.parametrize(
    "expected,example",
    [
        (set(), TypeSignature.factory(arg_types=None, return_type=None)),
        (
            {"str", "datetime"},
            TypeSignature.factory(
                arg_types=ArgTypes.factory(
                    name=ArgsSection.ARGS,
                    args=OrderedDict(
                        [("a", TypeAtom("str", [])), ("b", TypeAtom("datetime", []))]
                    ),
                ),
                return_type=None,
            ),
